...
# Reasoning models emit their chain of thought before the answer, in a variety
# of wrappers depending on the model and the server. Strip them all.
_THINK_BLOCK = re.compile(
    r"<(think|thinking|reasoning)>.*?</\1>", re.DOTALL | re.IGNORECASE
)
# An unterminated opener happens when the block is truncated by max_tokens.
_THINK_OPEN = re.compile(r"^.*?<(think|thinking|reasoning)>", re.DOTALL | re.IGNORECASE)

REQUIRED_KEYS = {"impact", "urgency", "category"}


def extract_decision_json(raw: str) -> dict:
    """Find the decision object in a response that may contain reasoning.

    The naive approach - first brace to last brace - breaks on reasoning
    models, because chain of thought routinely contains braces and the span
    then covers prose. This scans every balanced object in the text and
    returns the last one carrying the keys a decision must have.

    Choosing the LAST match matters. A model that reasons aloud often drafts a
    candidate object mid-thought and revises it. The final object is its answer.
    """
    text = _THINK_BLOCK.sub(" ", raw)
    if "<" in text and any(
        tag in text.lower() for tag in ("<think", "<thinking", "<reasoning")
    ):
        ...
        if char == '"':
            ...
            if depth == 0 and start != -1:
                    try:
                        ...
                        if isinstance(parsed, dict) and set(parsed) >= REQUIRED_KEYS:
                            candidates.append(parsed)
                    start = -1

    if not candidates:
        raise ClassifierError(
            "no decision object found in model output. "
...
class OpenAICompatClassifier:
    ...
    def classify(
        ...
        data = self._post(
            {
                ...
                "temperature": 0,
                "max_tokens": 2000,
                # Reasoning models default to thinking on, which costs tokens
                # and wraps the answer. Servers that do not recognise these
                # ignore them.
        ...
        payload = extract_decision_json(raw)
